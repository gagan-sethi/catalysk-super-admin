    ...
    if (!email || !validateEmail(email)) {
      ...
      try {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_API_ENDPOINT}/auth/forgotPassword`,
          {
            method: 'POST',
            headers: {
        ...
        if (data.code == 200) {
          console.log('test')
          setErrorMessage(
            'Link sent to registered mail id. Please check your mail.'
          )
        } else {
      ...
      <Col xxl={4} lg={6} md={8} xs={12} className='py-8 py-xl-0'>
        ...
        <Card className='smooth-shadow-md'>
          ...
          <Card.Body className='p-6'>
            <div className='mb-4'>
              <Link href='/'>
                {/* <Image src="/images/brand/logo/logo.svg" className="mb-3" height="50" alt="" /> */}
                <Image
