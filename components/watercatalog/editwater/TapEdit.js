...
const TapEdit = ({
  ...
  const validationSchema = Yup.object().shape({
    ...
    // stream_type: Yup.number("AEC must be number")
    // .required("AEC is required")
    // .positive("AEC must be positive"),
    use_for: Yup.string().required("Field should not empty"),
    ...
    flow_litre_per_minute: Yup.string().required("Speed of water is required"),
  });
  console.log("viewWaterProduct", viewWaterProduct.product_link);
    ...
    formik.validateForm().then(async (errors) => {
      if (Object.keys(errors).length === 0) {
        ...
        const response = await handleApi(
          `catalogue/editProducts/${viewWaterProduct._id}`,
          "PATCH",
          postdata,
          {},
          true,
  ...
  useEffect(() => {
    if (viewWaterProduct) {
      formik.setValues({
        brand: viewWaterProduct?.brand || "",
        comments: viewWaterProduct?.comments || "",
        // cfm: viewWaterProduct?.cfm || "",
        // stream_type: viewWaterProduct?.stream_type || "",
        use_for: viewWaterProduct?.use_for || "",
        price: viewWaterProduct?.price || "",
        customer_rating: viewWaterProduct?.customer_rating || "",
        no_of_ratings: viewWaterProduct?.no_of_ratings || "",
        // energy_efficiency_rating:
        // viewWaterProduct?.energy_efficiency_rating || "",
        efficient_percentage: viewWaterProduct?.efficient_percentage || "",
        // watt_per_hour: viewWaterProduct?.watt_per_hour || "",

        // product_link: viewWaterProduct.product_link || [""],
        product_link: Array.isArray(viewWaterProduct?.product_link)
          ? viewWaterProduct?.product_link
          : [viewWaterProduct?.product_link] || [""],

        // colour:
        //   Array.isArray(viewWaterProduct?.colour) &&
        //   viewWaterProduct?.colour.length > 0
        //     ? Array.isArray(viewWaterProduct?.colour) &&
        //       viewWaterProduct?.colour.length > 0
        //       ? [""]
        //       : viewWaterProduct?.colour
        //     : [viewWaterProduct?.colour] || [""],
        // colour: (viewWaterProduct?.colour && viewWaterProduct?.colour.length>0)?viewWaterProduct?.colour: [""],

        product_images: viewWaterProduct?.product_images || [""],
        // product_images: viewWaterProduct?.product_images || [""],

        // added value
        // blade_length: viewWaterProduct?.blade_length || "",
        // remote: viewWaterProduct?.remote || "",
        // price_on_company_site: viewWaterProduct?.price_on_company_site || "",
        // company_url: viewWaterProduct?.company_url || "",
        // model_name: viewWaterProduct?.model_name || "",
        flow_litre_per_minute: viewWaterProduct?.flow_litre_per_minute || "",
        // watt_per_hour: viewWaterProduct?.watt_per_hour || "",
      });
    }
  }, [viewWaterProduct]);

  console.log("efficient_percentage", viewWaterProduct);
              ...
              <div className="view-mdl only-view">
                ...
                <div className="row">
                  ...
                  {/* 
                        <div className="col-lg-6">
                    <div className="form-group mb-3">
                      <label className="mb-3">Stream Type</label>
                      <input
                        style={{
                          pointerEvents: "auto",
                          backgroundColor: "white",
                        ...
                        {...formik.getFieldProps("stream_type")}
                      />
                      {formik.touched.stream_type && formik.errors.stream_type && (
                        <small className="text-danger">
                          {formik.errors.stream_type}
                        </small>
                      )}
                    </div>
                  </div> */}
                  ...
                  <div className="col-lg-6">
                    <div className="form-group mb-3">
                      <label className="mb-3">Use For</label>
                      <select
                        style={{
                          pointerEvents: "auto",
                          backgroundColor: "white",
                        ...
                        {...formik.getFieldProps("use_for")}
                      >
                         <option value="Bathroom">Bathroom</option>
                        <option value="kitchen">kitchen</option>
                      </select>
                      {formik?.touched?.use_for && formik?.errors?.use_for && (
                        <small className="text-danger">
                          {formik?.errors.use_for}
                        </small>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="col-lg-6">
                    <div className="form-group mb-3">
                      <label className="mb-3">Flow Per Minute</label>
                      <input
                        style={{
                          pointerEvents: "auto",
                          backgroundColor: "white",
                        ...
                        type="text"
                        className="form-control"
                        {...formik.getFieldProps("flow_litre_per_minute")}
                      />
                      {formik?.touched?.flow_litre_per_minute &&
                        formik?.errors?.flow_litre_per_minute && (
                          <small className="text-danger">
                            {formik?.errors?.flow_litre_per_minute}
                          </small>
                        )}
